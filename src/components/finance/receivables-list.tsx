...
export function ReceivablesList({ receivables: initialReceivables }: { receivables: Receivable[] }) {
  ...
  async function handleConfirmPayment() {
    ...
    toast.success(`Marked ${payingRow.customer_name} as paid`)
    setReceivables((prev) => prev.filter((r) => r.id !== payingRow.id))
    setPayingId(null)
    router.refresh()
  }
          ...
          return (
            <Card key={r.id} className={cn("transition-colors hover:bg-accent/30", isOverdue && "border-destructive/40")}>
              <CardContent className="grid grid-cols-1 gap-2 p-4 sm:grid-cols-[1.3fr_1fr_1fr_1fr_1fr_100px_120px] sm:items-center sm:gap-4">
                ...
                <Button size="sm" variant="outline" onClick={() => openPayDialog(r.id)}>
          ...
          {payingRow && (
            <div className="space-y-4">
              <div className="rounded-lg border bg-muted/30 px-3 py-2">
                <p className="text-sm text-muted-foreground">{payingRow.customer_name}</p>
